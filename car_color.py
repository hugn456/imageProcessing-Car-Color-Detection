...
def computeCarColorRGBPixelArray(px_array_r, px_array_g, px_array_b, image_width, image_height):
    red_sum=0.0
    green_sum=0.0
    blue_sum=0.0
    for i in range(image_height):
        for j in range(image_width):
            red_sum+=px_array_r[i][j]
            green_sum+=px_array_g[i][j]
            blue_sum+=px_array_b[i][j]
    average_red_sum=  round(red_sum/(image_width*image_height))  
    average_green_sum=  round(green_sum/(image_width*image_height)) 
    average_blue_sum=  round(blue_sum/(image_width*image_height))     
    average_rgb_pixel=(average_red_sum,average_green_sum,average_blue_sum)
    
    car_color_array=createInitializedGreyscalePixelArray(image_width, image_height)

    for i in range(image_height):
        for j in range(image_width):
            car_color_array[i][j]=average_rgb_pixel
    return car_color_array
...
def main():
    ...
    for val in sorted_connected_list:
        ...
        if (abs(max_x-min_x)/abs(max_y-min_y)<=5 and abs(max_x-min_x)/abs(max_y-min_y)>=1.5):
            #or (abs(max_y-min_y)/abs(max_x-min_x)<=5 and abs(max_y-min_y)/abs(max_x-min_x)>=1.5)
            break
    print(connected_val)
